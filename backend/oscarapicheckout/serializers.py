...
class OrderSerializer(OscarOrderSerializer):
    ...
    class Meta:
        model = Order
        fields = OscarOrderSerializer.Meta.fields + ('stripe_payment_intent',)
...
class CheckoutSerializer(OscarCheckoutSerializer):
    ...
    stripe_payment_method = serializers.CharField(default='')
    ...
    def validate(self, data):
        ...
        p = data['basket'].lines.first().stockrecord.partner
        if p and not p.is_open:
            message = _("Partner is closed")
