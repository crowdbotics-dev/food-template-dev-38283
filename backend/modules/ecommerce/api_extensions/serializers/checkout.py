...
from consumer.api.v1.serializers import CustomProductSerializer, ProductOptionWithGroupSerializer
...
(InlineShippingAddressSerializer, InlineBillingAddressSerializer, InlineSurchargeSerializer,
 OrderOfferDiscountSerializer, OrderVoucherOfferSerializer) = get_api_classes(
    "serializers.checkout",
    ['InlineShippingAddressSerializer', 'InlineBillingAddressSerializer', 'InlineSurchargeSerializer',
     'OrderOfferDiscountSerializer', 'OrderVoucherOfferSerializer'])
...
class OrderLineAttributeSerializer(OscarModelSerializer):
    ...
    def get_group(self, obj):
        if obj and obj.group:
            qry = obj.group.options.filter(option=obj.option).first()
            serializer = ProductOptionWithGroupSerializer(qry)
