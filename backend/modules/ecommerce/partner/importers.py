...
from modules.ecommerce.catalogue.models import Option, ProductOptionGroup, ProductOption
...
class CatalogueImporter(object):
    ...
    def _import(self, file_path):
        ...
        if isinstance(file_path, InMemoryUploadedFile):
            file = file_path.read().decode('utf-8')
            import io
            reader = csv.DictReader(io.StringIO(file))
            for row in reader:
                row_number += 1
                # row = row.decode('utf-8').strip().split(self._delimiter)
                self._import_row(row_number, list(row.values()), stats)
        else:
    ...
    def _import_row(self, row_number, row, stats):
        if len(row) != 5 and len(row) != 9 and len(row) != 10:
        ...
        if len(row) in [9, 10]:
            ...
            if len(row) == 10:
                # With stock data
                self._create_options(item, *row[9:10], stats=stats)
    ...
    def _create_options(self, item, options, stats):
        # Create options
        clean = options.replace('\n', '')
        addons = clean.split(';')
        for addon in addons:
            if not addon:
                continue
            first = addon
            option_group = first.split('>')
            [name, min, max] = option_group[0].split(',')
            name = name.strip().capitalize()
            product_options, _ = ProductOptionGroup.objects.get_or_create(
                product=item, title=name
            )
            product_options.min = min
            product_options.max = max
            product_options.save()
            options = option_group[1].split('|')
            for option in options:
                name, price = option.split(',')
                name = name.strip().capitalize()
                option_obj, _ = Option.objects.get_or_create(name=name)
                group_option, _ = ProductOption.objects.get_or_create(
                    product_option_group=product_options, option=option_obj
                )
